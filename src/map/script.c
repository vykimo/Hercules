	...
	while( st->state == RUN ) {
		...
		switch(c) {
			...
			case C_ARG:
				...
				break;
			case C_STR:
				script->push_str(stack,C_CONSTSTR,(char*)&VECTOR_INDEX(st->script->script_buf, st->pos));
			...
			case C_LSTR:
			{
				...
				if( (!st->rid || !(lsd = map->id2sd(st->rid)) || !lsd->lang_id) && !map->default_lang_id )
					...
					script->push_str(stack,C_CONSTSTR,
							( k == translations ) ? script->string_list+string_id : *(char**)(&VECTOR_INDEX(st->script->script_buf, offset)) );
				}
...
void script_load_translations(void) {
	...
	if (total != 0) {
		...
		for (string_db = dbi_first(main_iter); dbi_exists(main_iter); string_db = dbi_next(main_iter)) {
			DBIterator *sub_iter = db_iterator(string_db);
