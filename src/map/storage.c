...
void guild_storage_delete(int guild_id) {
	...
	gs = idb_get(gstorage->db,guild_id);
	if (gs == NULL)
		return;
...
int storage_guild_storageopen(struct map_session_data* sd)
{
	...
	if((gstor = idb_get(gstorage->db,sd->status.guild_id)) == NULL || gstor->items == NULL) {
...
int storage_guild_storagesaved(int guild_id)
{
	...
	if((stor=idb_get(gstorage->db,guild_id)) != NULL) {
		if (stor->dirty && !stor->storage_status) {
...
bool storage_guild_storage_grow( struct guild *g ) {
	...
	gs = idb_get(gstorage->db,g->guild_id);
